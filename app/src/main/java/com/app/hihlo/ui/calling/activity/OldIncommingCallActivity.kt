...
class OldIncomingCallActivity :BaseActivity<ActivitySingleVideoCallBinding>(), CallEndBroadcast.CallEndCallback {
...
//                    ).show()
    ...
    private fun setupDoubleTapListeners() {
        binding.remoteUser.setOnTouchListener(object : View.OnTouchListener {
            private var dX = 0f
            private var dY = 0f
            private val gestureDetector = GestureDetector(this@OldIncomingCallActivity, object : GestureDetector.SimpleOnGestureListener() {
...
//            }
            ...
            val result = mRtcEngine?.joinChannel(agoraToken, channelName, Preferences.getCustomModelPreference<LoginResponse>(this@OldIncomingCallActivity, LOGIN_DATA)?.payload?.userId?.toInt() ?: 0, options)
...
//        mRtcEngine?.setupRemoteVideo(VideoCanvas(remoteTextureView, VideoCanvas.RENDER_MODE_FIT, remoteUid))
    ...
    private fun initializeRtcEngine() {
        if(mRtcEngine==null){
            try {
                val agoraAppId = Preferences.getCustomModelPreference<LoginResponse>(this@OldIncomingCallActivity, LOGIN_DATA)?.payload?.AGORA_DETAILS?.AGORA_APP_ID
    ...
    private val mRtcEventHandler = object : IRtcEngineEventHandler() {
        ...
        override fun onUserJoined(uid: Int, elapsed: Int) {
            ...
            runOnUiThread {
                ...
                stopBackgroundMusicService(this@OldIncomingCallActivity)
            }
        }
